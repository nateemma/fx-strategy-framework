...
def build_spot_panel(cache_dir, loader=load_series, codes=None) -> pd.DataFrame:
    ...
    if codes is None:
        codes = [c for c in CURRENCIES if c != "USD" and CURRENCIES[c].spot_fred is not None]
    cols = {}
    for code in codes:
        cur = CURRENCIES[code]
        if cur.spot_fred is None:
            raise ValueError(f"{code} has no FRED spot (IBKR-sourced); use forex.data.ibkr.build_carry_view")
