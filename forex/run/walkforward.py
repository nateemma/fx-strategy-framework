...
def walk_forward(strategy_factory, view, train_days, test_days, cost_bps: float = 1.0,
                 financing: bool = False) -> Result:
    cal = view.calendar
    rets_parts, wt_parts = [], []
    for train_sl, test_sl in wf_splits(cal, train_days, test_days):
        strat = strategy_factory()
        strat.fit(view.truncate(cal[train_sl][-1]))
        r = backtest(strat, view, cost_bps=cost_bps, financing=financing)
