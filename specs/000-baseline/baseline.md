...
| 6 | ~~Buy a futures market-data subscription~~ | S | **Closed** | Gated 2026-08-21 by probing the live Gateway. **Futures data already works** — all eight markets return bars, permissions are in place; the earlier 0-bars reading was a competing live session, which blocks SPY too. The real blocker is **history retention**: CONTFUT gives 2 years on the micros (M6E/M6A from 2025-06) and `includeExpired` returns only 8 contracts back to 2025-12. A subscription cannot buy contract-database retention. `docs/lean-data-gate.md` addendum. |
