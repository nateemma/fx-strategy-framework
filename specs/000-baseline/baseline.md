...
| 6 | **Buy the US Securities Snapshot and Futures Value Bundle ($10/month)** | S | **Highest** | The entire blocker on the trend sleeve. **Not** the *Futures Value Bundle PLUS* — that is the L2 depth add-on and it *requires* this base bundle (IBKR footnote 5), which is why the portal auto-adds it. The base bundle carries L1 top-of-book for CBOT/CME/COMEX/NYMEX, covering all eight markets. **Also needs US Futures Trading Permissions enabled** (footnote 3) — never granted on this account, and likely the real hurdle. Then re-run `scripts/trend_sleeve.py`: 0 bars means it refuses, bars mean spec `005` phase 6 is unblocked — check history *depth* first. |
