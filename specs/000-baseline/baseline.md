...
| 6 | ~~Buy a futures market-data subscription~~ | S | **Closed (provisional)** | Gated 2026-08-21 by probing the live Gateway. **Futures data already works** — all eight markets return bars; the earlier 0-bars reading was a competing live session, which blocks SPY too. The real blocker is **history retention**: CONTFUT gives 2 years on the micros (M6E/M6A from 2025-06) and `includeExpired` returns only 8 contracts back to 2025-12. A subscription cannot buy contract-database retention. **Measured while futures approval was still pending — re-run the probes once approved to confirm depth is unchanged.** `docs/lean-data-gate.md` addendum. |
