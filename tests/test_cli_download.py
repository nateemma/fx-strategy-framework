...
def test_run_download(monkeypatch):
    monkeypatch.setattr(refmod, "refresh_cache", lambda cache_dir, codes=None, loader=None, on_step=None: ["S1", "S2"])
...
def test_main_download_prints(monkeypatch, capsys):
    monkeypatch.setattr(refmod, "refresh_cache", lambda cache_dir, codes=None, loader=None, on_step=None: ["S1"])
