...
|---|---|---|---|---|
| **A1** | **VIX futures term-structure carry** | The volatility curve is in contango ~80% of the time; short-front/long-back harvests roll-down. Futures ⇒ leverage at ~risk-free, sidestepping filter 5 entirely. The single most-documented carry outside FX. | CBOE publishes settlement history free | Violent left tail (Feb 2018). Must be sized on that, not on average vol. |
| **A2** | **Cross-asset managed futures (proper)** | Only *commodity* trend was tested and found decayed. Full CTA trend across bonds, equity index, FX **and** commodities in one risk-parity book was never run — and the diversification across sleeves is the documented source of value in this program. | Free continuous futures + existing FRED | Trend is crowded; must be era-split. |
